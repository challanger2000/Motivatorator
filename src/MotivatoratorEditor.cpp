...
public:
    ...
    : CView(size), controller_(controller) {
        gnomiPositive_ = load("gnomi_positive.png");
        gnomiNegative_ = load("gnomi_negative.png");
        rockyPositive_ = load("rocky_positive.png");
        rockyNegative_ = load("rocky_negative.png");
        domPositive_ = load("dom_positive.png");
        domNegative_ = load("dom_negative.png");
    ...
    void draw(VSTGUI::CDrawContext* context) override {
        ...
        const double tone = controller_->getParamNormalized(kPhraseToneId);

        bool negative = mode == 1 || (mode == 2 && tone >= 0.5);
        VSTGUI::SharedPointer<VSTGUI::CBitmap> bitmap;
        if (character == 0)
            bitmap = negative ? gnomiNegative_ : gnomiPositive_;
        else if (character == 1)
            bitmap = negative ? rockyNegative_ : rockyPositive_;
        else
            bitmap = negative ? domNegative_ : domPositive_;
...
private:
    ...
    VSTGUI::SharedPointer<VSTGUI::CBitmap> gnomiPositive_, gnomiNegative_;
    VSTGUI::SharedPointer<VSTGUI::CBitmap> rockyPositive_, rockyNegative_;
    VSTGUI::SharedPointer<VSTGUI::CBitmap> domPositive_, domNegative_;
    VSTGUI::SharedPointer<VSTGUI::CVSTGUITimer> timer_;
};

static void drawButtonGlow(VSTGUI::CDrawContext* context, VSTGUI::CRect rect,
                           const VSTGUI::CColor& color) {
    rect.inset(2., 2.);
    VSTGUI::CColor fill = color; fill.alpha = 30;
    context->setFillColor(fill);
    context->drawRect(rect, VSTGUI::kDrawFilled);
    VSTGUI::CColor edge = color; edge.alpha = 210;
    context->setFrameColor(edge);
    context->setLineWidth(1.0);
    context->drawRect(rect, VSTGUI::kDrawStroked);
    auto inner = rect; inner.inset(2., 2.);
    VSTGUI::CColor innerEdge = color; innerEdge.alpha = 64;
...
public:
    ParameterButtonView(const VSTGUI::CRect& size, EditController* controller, ParamID paramId,
                        double value, const VSTGUI::CColor& glowColor)
    ...
    void draw(VSTGUI::CDrawContext* context) override {
        if (controller_ && std::abs(controller_->getParamNormalized(paramId_) - value_) < 0.20)
