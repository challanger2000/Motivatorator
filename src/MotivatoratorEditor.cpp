...
public:
    CharacterView(const VSTGUI::CRect& size, EditController* controller) : CView(size), controller_(controller) {
        gnomiPositive_=load("gnomi_positive.png"); gnomiNegative_=load("gnomi_negative.png"); rockyPositive_=load("rocky_positive.png"); rockyNegative_=load("rocky_negative.png"); domPositive_=load("dom_positive.png"); domNegative_=load("dom_negative.png");
        setMouseEnabled(false); timer_=VSTGUI::makeOwned<VSTGUI::CVSTGUITimer>([this](VSTGUI::CVSTGUITimer*){invalid();},100);
    }
    void draw(VSTGUI::CDrawContext* context) override {
        if(!controller_){setDirty(false);return;}
        const int mode=std::clamp(static_cast<int>(std::lround(controller_->getParamNormalized(kModeId)*2.0)),0,2); const int character=std::clamp(static_cast<int>(std::lround(controller_->getParamNormalized(kCharacterId)*2.0)),0,2); const bool negative=mode==1||(mode==2&&controller_->getParamNormalized(kPhraseToneId)>=0.5);
        VSTGUI::SharedPointer<VSTGUI::CBitmap> bitmap; if(character==0) bitmap=negative?gnomiNegative_:gnomiPositive_; else if(character==1) bitmap=negative?rockyNegative_:rockyPositive_; else bitmap=negative?domNegative_:domPositive_; if(bitmap) bitmap->draw(context,getViewSize(),VSTGUI::CPoint(0.,0.),1.f); setDirty(false);
    }
private:
    static VSTGUI::SharedPointer<VSTGUI::CBitmap> load(const char* name){auto bitmap=VSTGUI::makeOwned<VSTGUI::CBitmap>(VSTGUI::CResourceDescription(name));if(bitmap&&bitmap->getPlatformBitmap()) bitmap->getPlatformBitmap()->setScaleFactor(2.0);return bitmap;}
    EditController* controller_{nullptr}; VSTGUI::SharedPointer<VSTGUI::CBitmap> gnomiPositive_,gnomiNegative_,rockyPositive_,rockyNegative_,domPositive_,domNegative_; VSTGUI::SharedPointer<VSTGUI::CVSTGUITimer> timer_;
};

class ModeTitleView final : public VSTGUI::CView {
public:
    ModeTitleView(const VSTGUI::CRect& size, EditController* controller):CView(size),controller_(controller){setMouseEnabled(false);timer_=VSTGUI::makeOwned<VSTGUI::CVSTGUITimer>([this](VSTGUI::CVSTGUITimer*){invalid();},100);}
    void draw(VSTGUI::CDrawContext* context) override {
        if(!controller_){setDirty(false);return;} const int mode=std::clamp(static_cast<int>(std::lround(controller_->getParamNormalized(kModeId)*2.0)),0,2); const char* text=mode==0?"MOTIVATOR":(mode==1?"DEMOTIVATOR":"MIXED"); VSTGUI::CColor color=mode==0?VSTGUI::CColor(255,184,70,255):(mode==1?VSTGUI::CColor(238,72,45,255):VSTGUI::CColor(255,137,48,255)); const auto rect=getViewSize(); context->setFont(VSTGUI::makeOwned<VSTGUI::CFontDesc>("Arial",22.,VSTGUI::kBoldFace)); context->setDrawMode(VSTGUI::kAntiAliasing);
        VSTGUI::CColor glow=color; glow.alpha=52; context->setFontColor(glow); for(double dx=-2.;dx<=2.;dx+=2.)for(double dy=-2.;dy<=2.;dy+=2.){if(dx==0.&&dy==0.)continue;auto r=rect;r.offset(dx,dy);context->drawString(text,r,VSTGUI::kCenterText,true);} glow.alpha=105;context->setFontColor(glow);for(double dx=-1.;dx<=1.;dx+=2.)for(double dy=-1.;dy<=1.;dy+=2.){auto r=rect;r.offset(dx,dy);context->drawString(text,r,VSTGUI::kCenterText,true);} context->setFontColor(color);context->drawString(text,rect,VSTGUI::kCenterText,true);setDirty(false);
    }
...
public:
    ...
    void draw(VSTGUI::CDrawContext* context) override {
        if(!controller_){setDirty(false);return;}
        const int total=static_cast<int>(MotivatoratorPhrases::kPhraseCount*2); const int global=std::clamp(static_cast<int>(std::lround(controller_->getParamNormalized(kPhraseId)*(total-1))),0,total-1); const int perLanguage=static_cast<int>(MotivatoratorPhrases::kPhraseCount); const bool english=global>=perLanguage; const int local=global%perLanguage; const bool positive=local<static_cast<int>(MotivatoratorPhrases::kMotivatorCount); const int phraseIndex=positive?local:local-static_cast<int>(MotivatoratorPhrases::kMotivatorCount);
        const auto& phrase=positive?MotivatoratorPhrases::kMotivator[phraseIndex]:MotivatoratorPhrases::kDemotivator[phraseIndex]; const Steinberg::Vst::TChar* text=english?phrase.en:phrase.de;
        const double fontSize=chooseFontSize(text); auto font=VSTGUI::makeOwned<VSTGUI::CFontDesc>("Arial",fontSize,VSTGUI::kBoldFace); context->setFont(font); context->setDrawMode(VSTGUI::kAntiAliasing); VSTGUI::CColor color=positive?VSTGUI::CColor(255,174,62,255):VSTGUI::CColor(238,76,48,255);
        std::vector<std::u16string> lines=wrap(text,maxCharsFor(fontSize)); const double lineH=fontSize*1.28; const double totalH=lineH*lines.size(); const auto area=getViewSize(); double y=area.top+(area.getHeight()-totalH)*0.5;
        for(const auto& line:lines){VSTGUI::CRect r(area.left,y,area.right,y+lineH); VSTGUI::CColor glow=color;glow.alpha=42;context->setFontColor(glow);for(double dx=-2.;dx<=2.;dx+=2.)for(double dy=-2.;dy<=2.;dy+=2.){if(dx==0.&&dy==0.)continue;auto h=r;h.offset(dx,dy);context->drawString(reinterpret_cast<const VSTGUI::UTF8StringPtr>(toUtf8(line).c_str()),h,VSTGUI::kCenterText,true);} context->setFontColor(color);const auto utf8=toUtf8(line);context->drawString(utf8.c_str(),r,VSTGUI::kCenterText,true);y+=lineH;}
        setDirty(false);
    }
private:
    static double chooseFontSize(const Steinberg::Vst::TChar* text){size_t n=0;while(text[n])++n;if(n<=32)return 25.;if(n<=48)return 22.;if(n<=68)return 19.;return 17.;}
    static size_t maxCharsFor(double s){return s>=25.?27:(s>=22.?33:(s>=19.?39:45));}
