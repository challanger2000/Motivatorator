...
public:
    ...
    void draw(VSTGUI::CDrawContext* context) override {
        ...
        for(double dx=-2.;dx<=2.;dx+=2.) for(double dy=-2.;dy<=2.;dy+=2.) {
            if(dx==0.&&dy==0.) continue; auto r=rect; r.offset(dx,dy); context->drawString(text,r,VSTGUI::kCenterText,true);
        }
        glow.alpha=105; context->setFontColor(glow);
        for(double dx=-1.;dx<=1.;dx+=2.) for(double dy=-1.;dy<=1.;dy+=2.) {
            auto r=rect; r.offset(dx,dy); context->drawString(text,r,VSTGUI::kCenterText,true);
        }
...
public:
    ...
    void draw(VSTGUI::CDrawContext* context) override {
        ...
        for(;fontSize>=16.;fontSize-=2.) {
            ...
            const double totalH=fontSize*1.24*lines.size(); if(totalH<=safe.getHeight()) break;
        }
        ...
        for(const auto& line:lines) {
            VSTGUI::CRect r(safe.left,y,safe.right,y+lineH); const VSTGUI::UTF8String utf8(toUtf8(line));
            VSTGUI::CColor glow=color; glow.alpha=static_cast<uint8_t>(46.0*fade); context->setFontColor(glow);
            for(double dx=-2.;dx<=2.;dx+=2.) for(double dy=-2.;dy<=2.;dy+=2.) {
                if(dx==0.&&dy==0.) continue; auto h=r; h.offset(dx,dy); context->drawString(utf8,h,VSTGUI::kCenterText,true);
            }
...
private:
    static std::vector<std::u16string> wrapToWidth(VSTGUI::CDrawContext* context,const Steinberg::Vst::TChar* text,double maxWidth){
        ...
        while(pos<src.size()){
            while(pos<src.size()&&src[pos]==u' ')++pos; if(pos>=src.size())break;
            size_t end=src.find(u' ',pos); if(end==std::u16string::npos)end=src.size(); std::u16string word=src.substr(pos,end-pos);
    ...
    static std::string toUtf8(const std::u16string& s){
        std::string out; for(char16_t c:s){
    ...
    EditController* controller_{nullptr}; int lastPhrase_{-1}; int fadeTicks_{4};
    VSTGUI::SharedPointer<VSTGUI::CVSTGUITimer> timer_;
};
...
static void drawButtonGlow(VSTGUI::CDrawContext* context,VSTGUI::CRect rect,const VSTGUI::CColor& color){
    ...
    auto h2=rect; h2.inset(3.,3.); VSTGUI::CColor c2=color; c2.alpha=88; context->setFrameColor(c2); context->drawRect(h2,VSTGUI::kDrawStroked);
...
public:
    ParameterButtonView(const VSTGUI::CRect& size,EditController* controller,ParamID paramId,double value,const VSTGUI::CColor& glowColor):CView(size),controller_(controller),paramId_(paramId),value_(value),glowColor_(glowColor){
        setMouseEnabled(true); timer_=VSTGUI::makeOwned<VSTGUI::CVSTGUITimer>([this](VSTGUI::CVSTGUITimer*){invalid();},100);
    }
    void draw(VSTGUI::CDrawContext* context)override{
        if(controller_&&std::abs(controller_->getParamNormalized(paramId_)-value_)<0.20)drawButtonGlow(context,getViewSize(),glowColor_); setDirty(false);
    }
    VSTGUI::CMouseEventResult onMouseDown(VSTGUI::CPoint&,const VSTGUI::CButtonState&)override{
        if(!controller_)return VSTGUI::kMouseEventNotHandled; setParameter(controller_,paramId_,value_); invalid(); return VSTGUI::kMouseEventHandled;
    }
private:
    EditController* controller_{nullptr}; ParamID paramId_{0}; double value_{0.0}; VSTGUI::CColor glowColor_;
    VSTGUI::SharedPointer<VSTGUI::CVSTGUITimer> timer_;
};

class OptionsButtonView final : public VSTGUI::CView {
public:
    OptionsButtonView(const VSTGUI::CRect& size, EditController* controller) : CView(size), controller_(controller) {
        setMouseEnabled(true);
        timer_=VSTGUI::makeOwned<VSTGUI::CVSTGUITimer>([this](VSTGUI::CVSTGUITimer*){invalid();},100);
    }
    void draw(VSTGUI::CDrawContext* context) override {
        if(controller_ && controller_->getParamNormalized(kOptionsId)>=0.5)
            drawButtonGlow(context,getViewSize(),VSTGUI::CColor(255,156,38,255));
        setDirty(false);
    }
    VSTGUI::CMouseEventResult onMouseDown(VSTGUI::CPoint&,const VSTGUI::CButtonState&) override {
        if(!controller_) return VSTGUI::kMouseEventNotHandled;
        const double next=controller_->getParamNormalized(kOptionsId)>=0.5?0.0:1.0;
        setParameter(controller_,kOptionsId,next); invalid(); return VSTGUI::kMouseEventHandled;
    }
private:
    EditController* controller_{nullptr};
    VSTGUI::SharedPointer<VSTGUI::CVSTGUITimer> timer_;
...
public:
    OptionsPanelView(const VSTGUI::CRect& size, EditController* controller) : CView(size), controller_(controller) {
        setMouseEnabled(true);
        timer_=VSTGUI::makeOwned<VSTGUI::CVSTGUITimer>([this](VSTGUI::CVSTGUITimer*){invalid();},100);
    }
    void draw(VSTGUI::CDrawContext* context) override {
        ...
        context->setDrawMode(VSTGUI::kAntiAliasing);
        auto panel=getViewSize(); panel.inset(18.,20.);
        VSTGUI::CColor bg(8,8,7,238); context->setFillColor(bg); context->drawRect(panel,VSTGUI::kDrawFilled);
        VSTGUI::CColor frame(178,108,38,225); context->setFrameColor(frame); context->setLineWidth(1.0); context->drawRect(panel,VSTGUI::kDrawStroked);
        auto inner=panel; inner.inset(3.,3.); frame.alpha=75; context->setFrameColor(frame); context->drawRect(inner,VSTGUI::kDrawStroked);

        const VSTGUI::CColor text(255,186,78,255);
        context->setFont(VSTGUI::makeOwned<VSTGUI::CFontDesc>("Arial",19.,VSTGUI::kBoldFace)); context->setFontColor(text);
        VSTGUI::CRect title(panel.left,panel.top+7.,panel.right,panel.top+32.); context->drawString("OPTIONS",title,VSTGUI::kCenterText,true);

        context->setFont(VSTGUI::makeOwned<VSTGUI::CFontDesc>("Arial",13.,VSTGUI::kBoldFace));
        VSTGUI::CRect langLabel(panel.left+18.,panel.top+46.,panel.left+100.,panel.top+66.); context->drawString("LANGUAGE",langLabel,VSTGUI::kLeftText,true);
        drawChoice(context,languageRect(0),"DEUTSCH",languageIndex()==0); drawChoice(context,languageRect(1),"ENGLISH",languageIndex()==1);

        VSTGUI::CRect intervalLabel(panel.left+18.,panel.top+104.,panel.left+100.,panel.top+124.); context->drawString("INTERVAL",intervalLabel,VSTGUI::kLeftText,true);
        static const char* labels[6]={"5s","10s","15s","20s","25s","30s"};
        const int selected=intervalIndex(); for(int i=0;i<6;++i) drawChoice(context,intervalRect(i),labels[i],selected==i);
        setDirty(false);
    }
    VSTGUI::CMouseEventResult onMouseDown(VSTGUI::CPoint& where,const VSTGUI::CButtonState&) override {
        if(!isOpen()||!controller_) return VSTGUI::kMouseEventNotHandled;
        for(int i=0;i<2;++i) if(languageRect(i).pointInside(where)){setParameter(controller_,kLanguageId,(double)i);invalid();return VSTGUI::kMouseEventHandled;}
        for(int i=0;i<6;++i) if(intervalRect(i).pointInside(where)){setParameter(controller_,kIntervalId,(double)i/5.0);invalid();return VSTGUI::kMouseEventHandled;}
        return VSTGUI::kMouseEventHandled;
    }
private:
    bool isOpen() const { return controller_&&controller_->getParamNormalized(kOptionsId)>=0.5; }
    int languageIndex() const { return std::clamp(static_cast<int>(std::lround(controller_->getParamNormalized(kLanguageId))),0,1); }
    int intervalIndex() const { return std::clamp(static_cast<int>(std::lround(controller_->getParamNormalized(kIntervalId)*5.0)),0,5); }
    VSTGUI::CRect languageRect(int index) const {
        auto panel=getViewSize(); panel.inset(18.,20.); const double w=98.,gap=10.; const double left=panel.left+88.+index*(w+gap);
        return VSTGUI::CRect(left,panel.top+42.,left+w,panel.top+70.);
    }
    VSTGUI::CRect intervalRect(int index) const {
        auto panel=getViewSize(); panel.inset(18.,20.); const double gap=5.; const double total=panel.getWidth()-36.; const double w=(total-gap*5.)/6.; const double left=panel.left+18.+index*(w+gap);
        return VSTGUI::CRect(left,panel.top+128.,left+w,panel.top+158.);
    }
    void drawChoice(VSTGUI::CDrawContext* context,VSTGUI::CRect rect,const char* label,bool selected) const {
        VSTGUI::CColor fill=selected?VSTGUI::CColor(122,63,18,185):VSTGUI::CColor(24,22,18,220); context->setFillColor(fill); context->drawRect(rect,VSTGUI::kDrawFilled);
        VSTGUI::CColor edge=selected?VSTGUI::CColor(255,151,44,245):VSTGUI::CColor(116,83,50,190); context->setFrameColor(edge); context->setLineWidth(1.0); context->drawRect(rect,VSTGUI::kDrawStroked);
        if(selected){auto glow=rect;glow.inset(2.,2.);edge.alpha=80;context->setFrameColor(edge);context->drawRect(glow,VSTGUI::kDrawStroked);}
        context->setFont(VSTGUI::makeOwned<VSTGUI::CFontDesc>("Arial",12.,VSTGUI::kBoldFace)); context->setFontColor(selected?VSTGUI::CColor(255,194,93,255):VSTGUI::CColor(199,167,126,255)); context->drawString(label,rect,VSTGUI::kCenterText,true);
    }
    EditController* controller_{nullptr};
    VSTGUI::SharedPointer<VSTGUI::CVSTGUITimer> timer_;
};
...
VSTGUI::CView* MotivatoratorEditor::createView(const VSTGUI::UIAttributes& attributes,const VSTGUI::IUIDescription* description){
    if(const auto name=attributes.getAttributeValue(VSTGUI::IUIDescription::kCustomViewName)){
        ...
        if(*name=="PhraseView")return new PhraseView(VSTGUI::CRect(315.,90.,665.,330.),controller_);
        ...
        if(*name=="OptionsButton")return new OptionsButtonView(VSTGUI::CRect(580.,8.,710.,52.),controller_);
        if(*name=="OptionsPanel")return new OptionsPanelView(VSTGUI::CRect(315.,90.,665.,330.),controller_);
    }
