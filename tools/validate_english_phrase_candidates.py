#!/usr/bin/env python3
"""Static quality checks for the English phrase candidate batches.

This script NEVER generates or combines phrases. It checks the complete English
bank for count, exact duplicates, forbidden concrete references, length and
suspiciously similar entries.
"""
...
FILES = sorted(PHRASE_DIR.glob("CuratedEnglishBatch*.inc"))
...
def main() -> int:
    ...
    for path, lineno, section, text in rows:
        ...
        if len(words) > 16:
            print(f"WARN long ({len(words)} words): {path.name}:{lineno}: {text}")
    ...
    for section in ("motivator", "demotivator"):
        if counts[section] != 500:
            errors += 1
            print(f"ERROR {section} has {counts[section]} candidates; final English target is exactly 500")

    if errors:
        print(f"\nFAILED: {errors} issue(s) require curation.")
        return 1
    print("\nPASS: English structural/style checks clean. Native tone still requires editorial review.")
    return 0
